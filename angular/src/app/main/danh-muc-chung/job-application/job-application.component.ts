...
import { JobApplicationFacadeService } from './service/job-application.facade';
import { CreateJobApplicationActions, JobApplicationColumns, createJobApplicationTableConfig } from './modals/dynamic/dynamic-table';
import { JobApplicationRowView } from './interface/job-application';

import { JobApplicationDto, SearchInputDto } from '../../../proxy/dtos/models';
import { ActivatedRoute, Router } from '@angular/router';
import { CommonModule } from '@angular/common';

type JobApplicationSearchTerms = SearchInputDto & {
...
@Component({
  standalone: false,
  selector: 'vnx-job-application',
  templateUrl: './job-application.component.html',
  styleUrls: ['./job-application.component.less'],
})
export class JobApplicationComponent extends AppBaseComponent implements OnInit, OnDestroy {
  // #region ViewChild & Constructor
  @ViewChild('inlineCreateUpdate') inlineCreateUpdate: CreateUpdateJobApplicationModalComponent;
  constructor(
    injector: Injector,
    private facade: JobApplicationFacadeService,
  ...
  dataDisplay: JobApplicationRowView[] = [];
  totalRecords = 0;
  loading = false;
  selectedRows: JobApplicationRowView[] = [];

  // Search & filter state
  searchKeyword = '';
  showFilter = false;
  searchTerms: JobApplicationSearchTerms = {
    keyword: '',
  ...
  updateJobApplicationDto = {} as JobApplicationDto;
  // #region Menu & Dynamic Table Config

  // Table columns, actions,
  columns: DynamicTableColumn[] = JobApplicationColumns;
  actions: DynamicTableAction[] = CreateJobApplicationActions();
  config: DynamicTableConfig = createJobApplicationTableConfig();
        ...
        switchMap(() => {
          ...
          return this.facade
            ...
            .pipe(
              catchError(() => {
                // fallback on error
                return of({ total: 0, data: [] as JobApplicationRowView[] });
              }),
              finalize(() => {
                this.hideLoading();
    ...
    combineLatest([data$, params$, query$])
      ...
      .subscribe(([data, params, q]) => {
        ...
        if (mode === 'create') {
          ...
          this.viewMode = 'edit';
          if (entity) {
            this.updateJobApplicationDto = entity as any;
            this.triggerBuildForm();
          } else {
            this.viewMode = 'list';
  ...
  startEdit(row: JobApplicationRowView): void {
    if (!row) return;
    this.router.navigate(['edit', row.id], { relativeTo: this.route });
  }

  startView(row: JobApplicationRowView): void {
    if (!row) return;
  ...
  onAction(evt: { action: DynamicTableAction; row: JobApplicationRowView }) {
    ...
    switch (action.key) {
      ...
      case 'view':
        ...
        break;
      case 'delete':
        this.deleteJobApplication(row?.id);
        break;
      default:
        break;
    }
