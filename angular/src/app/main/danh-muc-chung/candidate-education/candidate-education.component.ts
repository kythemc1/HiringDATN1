...
import { CandidateEducationFacadeService } from './service/candidate-education.facade';
import { CreateCandidateEducationActions, CandidateEducationColumns, createCandidateEducationTableConfig } from './modals/dynamic/dynamic-table';
import { CandidateEducationRowView } from './interface/candidate-education';

import { CandidateEducationDto, SearchInputDto } from '../../../proxy/dtos/models';
import { ActivatedRoute, Router } from '@angular/router';
import { CommonModule } from '@angular/common';

type CandidateEducationSearchTerms = SearchInputDto & {
...
@Component({
  standalone: false,
  selector: 'vnx-candidate-education',
  templateUrl: './candidate-education.component.html',
  styleUrls: ['./candidate-education.component.less'],
})
export class CandidateEducationComponent extends AppBaseComponent implements OnInit, OnDestroy {
  // #region ViewChild & Constructor
  @ViewChild('inlineCreateUpdate') inlineCreateUpdate: CreateUpdateCandidateEducationModalComponent;
  constructor(
    injector: Injector,
    private facade: CandidateEducationFacadeService,
  ...
  dataDisplay: CandidateEducationRowView[] = [];
  totalRecords = 0;
  loading = false;
  selectedRows: CandidateEducationRowView[] = [];

  // Search & filter state
  searchKeyword = '';
  showFilter = false;
  searchTerms: CandidateEducationSearchTerms = {
    keyword: '',
  ...
  updateCandidateEducationDto = {} as CandidateEducationDto;
  // #region Menu & Dynamic Table Config

  // Table columns, actions,
  columns: DynamicTableColumn[] = CandidateEducationColumns;
  actions: DynamicTableAction[] = CreateCandidateEducationActions();
  config: DynamicTableConfig = createCandidateEducationTableConfig();
        ...
        switchMap(() => {
          ...
          return this.facade
            ...
            .pipe(
              catchError(() => {
                // fallback on error
                return of({ total: 0, data: [] as CandidateEducationRowView[] });
              }),
              finalize(() => {
                this.hideLoading();
    ...
    combineLatest([data$, params$, query$])
      ...
      .subscribe(([data, params, q]) => {
        ...
        if (mode === 'create') {
          ...
          this.updateCandidateEducationDto = {} as any;
          this.triggerBuildForm();
        } else if (mode === 'edit') {
          this.viewMode = 'edit';
          if (entity) {
            this.updateCandidateEducationDto = entity as any;
            this.triggerBuildForm();
          } else {
            this.viewMode = 'list';
            this.router.navigate(['/main/danh-muc-chung/candidate-education']);
          }
        } else if (mode === 'view') {
          this.viewMode = 'view';
          if (entity) {
  ...
  startEdit(row: CandidateEducationRowView): void {
    if (!row) return;
    this.router.navigate(['edit', row.id], { relativeTo: this.route });
  }

  startView(row: CandidateEducationRowView): void {
    if (!row) return;
  ...
  onChildSaved(shouldClose?: boolean): void {
    this.viewMode = 'list';
    this.router.navigate(['/main/danh-muc-chung/candidate-education']).then(() => this.apply());
  }
  ...
  deleteCandidateEducation(id: number): void {
    if (id == null) return;
  ...
  onAction(evt: { action: DynamicTableAction; row: CandidateEducationRowView }) {
    ...
    switch (action.key) {
      ...
      case 'view':
        ...
        break;
      case 'delete':
        this.deleteCandidateEducation(row?.id);
        break;
      default:
        break;
    }
