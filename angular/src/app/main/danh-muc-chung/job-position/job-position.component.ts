...
import { JobPositionFacadeService } from './service/job-position.facade';
import { CreateJobPositionActions, JobPositionColumns, createJobPositionTableConfig } from './modals/dynamic/dynamic-table';
import { JobPositionRowView } from './interface/job-position';

import { JobPositionDto, SearchInputDto } from '../../../proxy/dtos/models';
import { ActivatedRoute, Router } from '@angular/router';
import { CommonModule } from '@angular/common';

type JobPositionSearchTerms = SearchInputDto & {
...
@Component({
  standalone: false,
  selector: 'vnx-job-position',
  templateUrl: './job-position.component.html',
  styleUrls: ['./job-position.component.less'],
})
export class JobPositionComponent extends AppBaseComponent implements OnInit, OnDestroy {
  // #region ViewChild & Constructor
  @ViewChild('inlineCreateUpdate') inlineCreateUpdate: CreateUpdateJobPositionModalComponent;
  constructor(
    injector: Injector,
    private facade: JobPositionFacadeService,
  ...
  dataDisplay: JobPositionRowView[] = [];
  totalRecords = 0;
  loading = false;
  selectedRows: JobPositionRowView[] = [];

  // Search & filter state
  searchKeyword = '';
  showFilter = false;
  searchTerms: JobPositionSearchTerms = {
    keyword: '',
  ...
  updateJobPositionDto = {} as JobPositionDto;
  // #region Menu & Dynamic Table Config

  // Table columns, actions,
  columns: DynamicTableColumn[] = JobPositionColumns;
  actions: DynamicTableAction[] = CreateJobPositionActions();
  config: DynamicTableConfig = createJobPositionTableConfig();
        ...
        switchMap(() => {
          ...
          return this.facade
            ...
            .pipe(
              catchError(() => {
                // fallback on error
                return of({ total: 0, data: [] as JobPositionRowView[] });
              }),
              finalize(() => {
                this.hideLoading();
    ...
    combineLatest([data$, params$, query$])
      ...
      .subscribe(([data, params, q]) => {
        ...
        if (mode === 'create') {
          ...
          this.viewMode = 'edit';
          if (entity) {
            this.updateJobPositionDto = entity as any;
            this.triggerBuildForm();
          } else {
            this.viewMode = 'list';
  ...
  startEdit(row: JobPositionRowView): void {
    if (!row) return;
    this.router.navigate(['edit', row.id], { relativeTo: this.route });
  }

  startView(row: JobPositionRowView): void {
    if (!row) return;
  ...
  onAction(evt: { action: DynamicTableAction; row: JobPositionRowView }) {
