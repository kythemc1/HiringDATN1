...
import { CandidateProjectFacadeService } from './service/candidate-project.facade';
import { CreateCandidateProjectActions, CandidateProjectColumns, createCandidateProjectTableConfig } from './modals/dynamic/dynamic-table';
import { CandidateProjectRowView } from './interface/candidate-project';

import { CandidateProjectDto, SearchInputDto } from '../../../proxy/dtos/models';
import { ActivatedRoute, Router } from '@angular/router';
import { CommonModule } from '@angular/common';

type CandidateProjectSearchTerms = SearchInputDto & {
...
@Component({
  standalone: false,
  selector: 'vnx-candidate-project',
  templateUrl: './candidate-project.component.html',
  styleUrls: ['./candidate-project.component.less'],
})
export class CandidateProjectComponent extends AppBaseComponent implements OnInit, OnDestroy {
  // #region ViewChild & Constructor
  @ViewChild('inlineCreateUpdate') inlineCreateUpdate: CreateUpdateCandidateProjectModalComponent;
  constructor(
    injector: Injector,
    private facade: CandidateProjectFacadeService,
  ...
  dataDisplay: CandidateProjectRowView[] = [];
  totalRecords = 0;
  loading = false;
  selectedRows: CandidateProjectRowView[] = [];

  // Search & filter state
  searchKeyword = '';
  showFilter = false;
  searchTerms: CandidateProjectSearchTerms = {
    keyword: '',
  ...
  updateCandidateProjectDto = {} as CandidateProjectDto;
  // #region Menu & Dynamic Table Config

  // Table columns, actions,
  columns: DynamicTableColumn[] = CandidateProjectColumns;
  actions: DynamicTableAction[] = CreateCandidateProjectActions();
  config: DynamicTableConfig = createCandidateProjectTableConfig();
        ...
        switchMap(() => {
          ...
          return this.facade
            ...
            .pipe(
              catchError(() => {
                // fallback on error
                return of({ total: 0, data: [] as CandidateProjectRowView[] });
              }),
              finalize(() => {
                this.hideLoading();
    ...
    combineLatest([data$, params$, query$])
      ...
      .subscribe(([data, params, q]) => {
        ...
        if (mode === 'create') {
          ...
          this.updateCandidateProjectDto = {} as any;
          this.triggerBuildForm();
        } else if (mode === 'edit') {
          this.viewMode = 'edit';
          if (entity) {
            this.updateCandidateProjectDto = entity as any;
            this.triggerBuildForm();
          } else {
            this.viewMode = 'list';
            this.router.navigate(['/main/danh-muc-chung/candidate-project']);
          }
        } else if (mode === 'view') {
          this.viewMode = 'view';
          if (entity) {
  ...
  startEdit(row: CandidateProjectRowView): void {
    if (!row) return;
    this.router.navigate(['edit', row.id], { relativeTo: this.route });
  }

  startView(row: CandidateProjectRowView): void {
    if (!row) return;
  ...
  onChildSaved(shouldClose?: boolean): void {
    this.viewMode = 'list';
    this.router.navigate(['/main/danh-muc-chung/candidate-project']).then(() => this.apply());
  }
  ...
  deleteCandidateProject(id: number): void {
    if (id == null) return;
  ...
  onAction(evt: { action: DynamicTableAction; row: CandidateProjectRowView }) {
    ...
    switch (action.key) {
      ...
      case 'view':
        ...
        break;
      case 'delete':
        this.deleteCandidateProject(row?.id);
        break;
      default:
        break;
    }
