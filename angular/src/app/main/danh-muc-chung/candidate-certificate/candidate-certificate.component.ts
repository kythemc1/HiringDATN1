...
import { CandidateCertificateFacadeService } from './service/candidate-certificate.facade';
import { CreateCandidateCertificateActions, CandidateCertificateColumns, createCandidateCertificateTableConfig } from './modals/dynamic/dynamic-table';
import { CandidateCertificateRowView } from './interface/candidate-certificate';

import { CandidateCertificateDto, SearchInputDto } from '../../../proxy/dtos/models';
import { ActivatedRoute, Router } from '@angular/router';
import { CommonModule } from '@angular/common';

type CandidateCertificateSearchTerms = SearchInputDto & {
...
@Component({
  standalone: false,
  selector: 'vnx-candidate-certificate',
  templateUrl: './candidate-certificate.component.html',
  styleUrls: ['./candidate-certificate.component.less'],
})
export class CandidateCertificateComponent extends AppBaseComponent implements OnInit, OnDestroy {
  // #region ViewChild & Constructor
  @ViewChild('inlineCreateUpdate') inlineCreateUpdate: CreateUpdateCandidateCertificateModalComponent;
  constructor(
    injector: Injector,
    private facade: CandidateCertificateFacadeService,
  ...
  dataDisplay: CandidateCertificateRowView[] = [];
  totalRecords = 0;
  loading = false;
  selectedRows: CandidateCertificateRowView[] = [];

  // Search & filter state
  searchKeyword = '';
  showFilter = false;
  searchTerms: CandidateCertificateSearchTerms = {
    keyword: '',
  ...
  updateCandidateCertificateDto = {} as CandidateCertificateDto;
  // #region Menu & Dynamic Table Config

  // Table columns, actions,
  columns: DynamicTableColumn[] = CandidateCertificateColumns;
  actions: DynamicTableAction[] = CreateCandidateCertificateActions();
  config: DynamicTableConfig = createCandidateCertificateTableConfig();
        ...
        switchMap(() => {
          ...
          return this.facade
            ...
            .pipe(
              catchError(() => {
                // fallback on error
                return of({ total: 0, data: [] as CandidateCertificateRowView[] });
              }),
              finalize(() => {
                this.hideLoading();
    ...
    combineLatest([data$, params$, query$])
      ...
      .subscribe(([data, params, q]) => {
        ...
        if (mode === 'create') {
          ...
          this.updateCandidateCertificateDto = {} as any;
          this.triggerBuildForm();
        } else if (mode === 'edit') {
          this.viewMode = 'edit';
          if (entity) {
            this.updateCandidateCertificateDto = entity as any;
            this.triggerBuildForm();
          } else {
            this.viewMode = 'list';
            this.router.navigate(['/main/danh-muc-chung/candidate-certificate']);
          }
        } else if (mode === 'view') {
          this.viewMode = 'view';
          if (entity) {
  ...
  startEdit(row: CandidateCertificateRowView): void {
    if (!row) return;
    this.router.navigate(['edit', row.id], { relativeTo: this.route });
  }

  startView(row: CandidateCertificateRowView): void {
    if (!row) return;
  ...
  onChildSaved(shouldClose?: boolean): void {
    this.viewMode = 'list';
    this.router.navigate(['/main/danh-muc-chung/candidate-certificate']).then(() => this.apply());
  }
  ...
  deleteCandidateCertificate(id: number): void {
    if (id == null) return;
  ...
  onAction(evt: { action: DynamicTableAction; row: CandidateCertificateRowView }) {
    ...
    switch (action.key) {
      ...
      case 'view':
        ...
        break;
      case 'delete':
        this.deleteCandidateCertificate(row?.id);
        break;
      default:
        break;
    }
