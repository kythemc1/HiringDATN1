...
import { AiPromptTemplateFacadeService } from './service/ai-prompt-template.facade';
import { CreateAiPromptTemplateActions, AiPromptTemplateColumns, createAiPromptTemplateTableConfig } from './modals/dynamic/dynamic-table';
import { AiPromptTemplateRowView } from './interface/ai-prompt-template';

import { AiPromptTemplateDto, SearchInputDto } from '../../../proxy/dtos/models';
import { ActivatedRoute, Router } from '@angular/router';
import { CommonModule } from '@angular/common';

type AiPromptTemplateSearchTerms = SearchInputDto & {
...
@Component({
  standalone: false,
  selector: 'vnx-ai-prompt-template',
  templateUrl: './ai-prompt-template.component.html',
  styleUrls: ['./ai-prompt-template.component.less'],
})
export class AiPromptTemplateComponent extends AppBaseComponent implements OnInit, OnDestroy {
  // #region ViewChild & Constructor
  @ViewChild('inlineCreateUpdate') inlineCreateUpdate: CreateUpdateAiPromptTemplateModalComponent;
  constructor(
    injector: Injector,
    private facade: AiPromptTemplateFacadeService,
  ...
  dataDisplay: AiPromptTemplateRowView[] = [];
  totalRecords = 0;
  loading = false;
  selectedRows: AiPromptTemplateRowView[] = [];

  // Search & filter state
  searchKeyword = '';
  showFilter = false;
  searchTerms: AiPromptTemplateSearchTerms = {
    keyword: '',
  ...
  updateAiPromptTemplateDto = {} as AiPromptTemplateDto;
  // #region Menu & Dynamic Table Config

  // Table columns, actions,
  columns: DynamicTableColumn[] = AiPromptTemplateColumns;
  actions: DynamicTableAction[] = CreateAiPromptTemplateActions();
  config: DynamicTableConfig = createAiPromptTemplateTableConfig();
        ...
        switchMap(() => {
          ...
          return this.facade
            ...
            .pipe(
              catchError(() => {
                // fallback on error
                return of({ total: 0, data: [] as AiPromptTemplateRowView[] });
              }),
              finalize(() => {
                this.hideLoading();
    ...
    combineLatest([data$, params$, query$])
      ...
      .subscribe(([data, params, q]) => {
        ...
        if (mode === 'create') {
          ...
          this.updateAiPromptTemplateDto = {} as any;
          this.triggerBuildForm();
        } else if (mode === 'edit') {
          this.viewMode = 'edit';
          if (entity) {
            this.updateAiPromptTemplateDto = entity as any;
            this.triggerBuildForm();
          } else {
            this.viewMode = 'list';
            this.router.navigate(['/main/danh-muc-chung/ai-prompt-template']);
          }
        } else if (mode === 'view') {
          this.viewMode = 'view';
          if (entity) {
  ...
  startEdit(row: AiPromptTemplateRowView): void {
    if (!row) return;
    this.router.navigate(['edit', row.id], { relativeTo: this.route });
  }

  startView(row: AiPromptTemplateRowView): void {
    if (!row) return;
  ...
  onChildSaved(shouldClose?: boolean): void {
    this.viewMode = 'list';
    this.router.navigate(['/main/danh-muc-chung/ai-prompt-template']).then(() => this.apply());
  }
  ...
  deleteAiPromptTemplate(id: number): void {
    if (id == null) return;
  ...
  onAction(evt: { action: DynamicTableAction; row: AiPromptTemplateRowView }) {
    ...
    switch (action.key) {
      ...
      case 'view':
        ...
        break;
      case 'delete':
        this.deleteAiPromptTemplate(row?.id);
        break;
      default:
        break;
    }
