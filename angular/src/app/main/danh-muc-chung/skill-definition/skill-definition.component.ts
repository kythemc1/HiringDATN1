...
import { SkillDefinitionFacadeService } from './service/skill-definition.facade';
import { CreateSkillDefinitionActions, SkillDefinitionColumns, createSkillDefinitionTableConfig } from './modals/dynamic/dynamic-table';
import { SkillDefinitionRowView } from './interface/skill-definition';

import { SkillDefinitionDto, SearchInputDto } from '../../../proxy/dtos/models';
import { ActivatedRoute, Router } from '@angular/router';
import { CommonModule } from '@angular/common';

type SkillDefinitionSearchTerms = SearchInputDto & {
...
@Component({
  standalone: false,
  selector: 'vnx-skill-definition',
  templateUrl: './skill-definition.component.html',
  styleUrls: ['./skill-definition.component.less'],
})
export class SkillDefinitionComponent extends AppBaseComponent implements OnInit, OnDestroy {
  // #region ViewChild & Constructor
  @ViewChild('inlineCreateUpdate') inlineCreateUpdate: CreateUpdateSkillDefinitionModalComponent;
  constructor(
    injector: Injector,
    private facade: SkillDefinitionFacadeService,
  ...
  dataDisplay: SkillDefinitionRowView[] = [];
  totalRecords = 0;
  loading = false;
  selectedRows: SkillDefinitionRowView[] = [];

  // Search & filter state
  searchKeyword = '';
  showFilter = false;
  searchTerms: SkillDefinitionSearchTerms = {
    keyword: '',
  ...
  updateSkillDefinitionDto = {} as SkillDefinitionDto;
  // #region Menu & Dynamic Table Config

  // Table columns, actions,
  columns: DynamicTableColumn[] = SkillDefinitionColumns;
  actions: DynamicTableAction[] = CreateSkillDefinitionActions();
  config: DynamicTableConfig = createSkillDefinitionTableConfig();
        ...
        switchMap(() => {
          ...
          return this.facade
            ...
            .pipe(
              catchError(() => {
                // fallback on error
                return of({ total: 0, data: [] as SkillDefinitionRowView[] });
              }),
              finalize(() => {
                this.hideLoading();
    ...
    combineLatest([data$, params$, query$])
      ...
      .subscribe(([data, params, q]) => {
        ...
        if (mode === 'create') {
          ...
          this.viewMode = 'edit';
          if (entity) {
            this.updateSkillDefinitionDto = entity as any;
            this.triggerBuildForm();
          } else {
            this.viewMode = 'list';
  ...
  startEdit(row: SkillDefinitionRowView): void {
    if (!row) return;
    this.router.navigate(['edit', row.id], { relativeTo: this.route });
  }

  startView(row: SkillDefinitionRowView): void {
    if (!row) return;
  ...
  deleteSkillDefinition(id: number): void {
    if (id == null) return;
  ...
  onAction(evt: { action: DynamicTableAction; row: SkillDefinitionRowView }) {
    ...
    switch (action.key) {
      ...
      case 'view':
        ...
        break;
      case 'delete':
        this.deleteSkillDefinition(row?.id);
        break;
      default:
        break;
    }
