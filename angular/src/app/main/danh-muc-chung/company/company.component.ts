...
import { CompanyFacadeService } from './service/company.facade';
import { CreateCompanyActions, CompanyColumns, createCompanyTableConfig } from './modals/dynamic/dynamic-table';
import { CompanyRowView } from './interface/company';

import { CompanyDto, SearchInputDto } from '../../../proxy/dtos/models';
import { ActivatedRoute, Router } from '@angular/router';
import { CommonModule } from '@angular/common';

type CompanySearchTerms = SearchInputDto & {
...
@Component({
  standalone: false,
  selector: 'vnx-company',
  templateUrl: './company.component.html',
  styleUrls: ['./company.component.less'],
})
export class CompanyComponent extends AppBaseComponent implements OnInit, OnDestroy {
  // #region ViewChild & Constructor
  @ViewChild('inlineCreateUpdate') inlineCreateUpdate: CreateUpdateCompanyModalComponent;
  constructor(
    injector: Injector,
    private facade: CompanyFacadeService,
  ...
  dataDisplay: CompanyRowView[] = [];
  totalRecords = 0;
  loading = false;
  selectedRows: CompanyRowView[] = [];
  ...
  updateCompanyDto = {} as CompanyDto;
  // #region Menu & Dynamic Table Config

  // Table columns, actions,
  columns: DynamicTableColumn[] = CompanyColumns;
  actions: DynamicTableAction[] = CreateCompanyActions();
  config: DynamicTableConfig = createCompanyTableConfig();
        ...
        switchMap(() => {
          ...
          return this.facade
            ...
            .pipe(
              catchError(() => {
                // fallback on error
                return of({ total: 0, data: [] as CompanyRowView[] });
              }),
              finalize(() => {
                this.hideLoading();
    ...
    combineLatest([data$, params$, query$])
      ...
      .subscribe(([data, params, q]) => {
        ...
        if (mode === 'create') {
          ...
          this.viewMode = 'edit';
          if (entity) {
            this.updateCompanyDto = entity as any;
            this.triggerBuildForm();
          } else {
            this.viewMode = 'list';
  ...
  startEdit(row: CompanyRowView): void {
    if (!row) return;
    this.router.navigate(['edit', row.id], { relativeTo: this.route });
  }

  startView(row: CompanyRowView): void {
    if (!row) return;
  ...
  onAction(evt: { action: DynamicTableAction; row: CompanyRowView }) {
