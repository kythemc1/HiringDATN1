...
@Component({
  standalone: false,
  selector: 'app-create-update-job-posting-modal',
  templateUrl: './create-update-job-posting-modal.component.html',
  styleUrls: ['./create-update-job-posting-modal.component.less'],
})
export class CreateUpdateJobPostingModalComponent
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if ('updateJobPostingDto' in changes && this.inline) {
      this.buildForm();
    }
  }
  ...
  save(): void {
    ...
    if (formValue.tenJobPosting) {
      formValue.tenJobPosting = formValue.tenJobPosting.trim();
    }
    ...
    const request = this.updateJobPostingDto.id
      ? this.JobPostionRecruitmentService.update(this.updateJobPostingDto.id, formValue)
      : this.JobPostionRecruitmentService.create(formValue);
    this.showLoading();
    request
      .pipe(
        takeWhile(() => this.componentActive),
        tap(() => {
          const isEdit = !!this.updateJobPostingDto.id;
  ...
  saveAndNew(): void {
    ...
    const isUpdate = !!this.updateJobPostingDto.id;
    const request = isUpdate
      ? this.JobPostionRecruitmentService.update(this.updateJobPostingDto.id, formValue as CreateUpdateJobPostingDto)
      : this.JobPostionRecruitmentService.create(formValue as CreateUpdateJobPostingDto);
    ...
    request
      .pipe(
        ...
        tap(() => {
          ...
          this.updateJobPostingDto = {} as JobPostingDto;
          this.form.reset();
