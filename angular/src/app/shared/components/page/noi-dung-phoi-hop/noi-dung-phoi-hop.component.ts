...
@Component({
  standalone: false,
  selector: 'app-noi-dung-phoi-hop',
  templateUrl: './noi-dung-phoi-hop.component.html',
  styleUrls: ['./noi-dung-phoi-hop.component.less'],
})
export class NoiDungPhoiHopComponent
  ...
  implements OnInit, OnChanges
{
  ...
  onSendReply(row: TnpcItem, parent?: TnpcItem): void {
    try {
      ...
      // Lấy tối đa 3 file trả lời để gửi kèm
      const filesToSend = (Array.isArray(row.filesReply) ? row.filesReply : []).slice(0, 3);
  ...
  onFileSelectBottom(event: Event, listName: 'toTrinh' | 'congVan' | 'dinhKem'): void {
    onFileSelectBottomSection(event, listName, {
      ...
      onSuccess: (mapped, input) => {
        switch (listName) {
          case 'toTrinh':
            this.filesToTrinh = [...this.filesToTrinh, ...mapped];
            break;
          case 'congVan':
            this.filesCongVan = [...this.filesCongVan, ...mapped];
            break;
          case 'dinhKem':
            this.filesDinhKem = [...this.filesDinhKem, ...mapped];
            break;
        }
        this.recomputeDisplayFromDtos();
