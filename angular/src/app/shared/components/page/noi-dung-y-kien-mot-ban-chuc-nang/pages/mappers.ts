...
const pickFiles = (dto: any) => toArr(dto?.fileDinhKems ?? dto?.fileDinhKemDtos ?? dto?.attachments).map(mapFile);
const findDto = (dtos: any[] | null | undefined, tcbcId: any, dmId: any) => {
  const items = toArr(dtos);
  return (
    items.find(x => x?.tieuChiBaoCaoId === tcbcId && x?.deMucId === dmId) ||
    items.find(x => x?.tieuChiBaoCaoId === tcbcId) ||
    items.find(x => String(x?.tieuChiBaoCaoId) === String(tcbcId) && String(x?.deMucId) === String(dmId))
  );
};
      ...
      child.children = [
        {
          ...
          replyContent: parentIsXuLy ? (replyFromInput ?? defaultReply) : '',
          isXuLy: parentIsXuLy,
          deMucId: deMucId,
          filesReply: pickFiles(
            findDto(
              parentIsXuLy ? noiDungXuLyDtosInput : noiDungPhoiHopDtosInput,
              !Number.isNaN(leafNumericId) ? leafNumericId : undefined,
              deMucId ?? undefined
            )
          ),
