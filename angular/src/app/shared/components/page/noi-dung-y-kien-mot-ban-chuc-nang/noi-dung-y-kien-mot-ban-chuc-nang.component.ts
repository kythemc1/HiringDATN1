...
@Component({
  standalone: false,
  selector: 'app-noi-dung-y-kien-mot-ban-chuc-nang',
  templateUrl: './noi-dung-y-kien-mot-ban-chuc-nang.component.html',
  styleUrls: ['./noi-dung-y-kien-mot-ban-chuc-nang.component.less'],
})
export class NoiDungYKienMotBanChucNangComponent
  ...
  implements OnInit, OnChanges
{
  ...
  @Output() emitReplyFiles = new EventEmitter<
    { tieuChiBaoCaoId: number; deMucId: number | null; fileDinhKems: CreateUpdateFileDinhKemDto[] }[]
  >();
  ...
  private getBanLabels(ids: Array<number | string> | null | undefined): string[] {
    ...
    // Normalize keys to string to handle both numeric and string IDs
  ...
  onFileSelectReply(event: Event, row: TnpcItem): void {
    ...
    // Emit updated reply files snapshot after upload selection
    setTimeout(() => {
      this.emitReplyFiles.emit(this.buildReplyFilesPayload());
    }, 0);
  }

  removeReplyFile(row: TnpcItem, index: number): void {
    removeReplyFileFromRow(row, index, { disabled: this.disabled, cdr: this.cdr });
    // Emit updated reply files snapshot after removal
    this.emitReplyFiles.emit(this.buildReplyFilesPayload());
  }

  // Gửi nội dung trả lời
  onSendReply(row: TnpcItem, parent?: TnpcItem): void {
    try {
      if (this.disabled) return;
      const baoCaoId = Number(this.baoCaoId);
      const tieuChiBaoCaoId = Number((row as any)?.tieuChiBaoCaoId);
      const deMucId = Number((row as any)?.deMucId);
      const content = (row.replyContent || '').toString().trim();
      const isXuLy = !!(row as any)?.isXuLy; // cờ xử lý theo cấp isDeMuc đã map xuống node cấp 2

      if (!baoCaoId || Number.isNaN(baoCaoId)) {
        this.showErrorMessage('Thiếu baoCaoId khi gửi nội dung trả lời');
        return;
      }
      if (!tieuChiBaoCaoId || Number.isNaN(tieuChiBaoCaoId)) {
        this.showErrorMessage('Thiếu tieuChiBaoCaoId khi gửi nội dung trả lời');
        return;
      }
      if (!deMucId || Number.isNaN(deMucId)) {
        this.showErrorMessage('Thiếu deMucId khi gửi nội dung trả lời');
        return;
      }
      if (!content && !(Array.isArray(row.filesReply) && row.filesReply.length)) {
        this.showErrorMessage('Vui lòng nhập nội dung hoặc đính kèm ít nhất 1 file');
        return;
      }

      const filesToSend = (Array.isArray(row.filesReply) ? row.filesReply : []);
      const fileDinhKems: CreateUpdateFileDinhKemDto[] = filesToSend.map(f => ({
        id: f?.id ? Number(f.id) : 0,
        duongDanFile: f?.url,
        tenFile: f?.name,
      }));

      const payload: CreateNoiDungTraLoiForTieuChiByChuyenVienDto = {
        id: 0,
        createNoiDungTraLoiItemDto: [
          {
            baoCaoId,
            deMucId,
            tieuChiBaoCaoId,
        ...
        // Đính kèm thêm file ở cấp payload (nếu backend xử lý)
        fileDinhKems: [],
      };
      this.showLoading();
      this.chuyenVienXuLyCongViecService.createNoiDungTraLoiForTieuChi(payload).subscribe({
        next: res => {
          this.hideLoading();
          const parentIsXuLy = !!(row as any)?.parentIsXuLy;
          // Clear form
          (row as any).replyContent = '';
          if (Array.isArray(row.filesReply)) {
            row.filesReply = [];
          }
          // Emit to parent when phối hợp to update noiDungPhoiHopDtos
          if (!parentIsXuLy) {
            const now = new Date();
            const dto: NoiDungPhoiHopDto = {
              baoCaoId,
              deMucId,
              tieuChiBaoCaoId,
              nguoiThucHienId: '',
              ngayThucHien: now.toISOString(),
              noiDung: content,
              toChucId: 0,
              isLast: true,
            };
            this.sentPhoiHop.emit(dto);
          }
          this.cdr.markForCheck();
          this.showSuccessMessage('Gửi nội dung thành công');
        },
        error: err => {
          this.showErrorMessage('Gửi nội dung thất bại');
        },
      });
    } catch (e) {
      console.error('onSendReply exception', e);
    }
  }

  // Upload for bottom attachments section
  onFileSelectBottom(event: Event, listName: 'toTrinh' | 'congVan' | 'dinhKem'): void {
  ...
  removeBottomFile(listName: 'toTrinh' | 'congVan' | 'dinhKem', index: number): void {
    removeBottomFileInLists(
      listName,
      index,
      {
  ...
  public updateAttachmentVisibility(): void {
    ...
    const nhieu = !!this.isNhieuBanXuLy;
    ...
    if (!xin && !nhieu) {
      ...
      // Chỉ hiển thị file đính kèm
      this.showToTrinh = false;
      this.showCongVan = false;
      this.showDinhKem = true;
    } else if (xin && !nhieu) {
      // Hiển thị tờ trình và đính kèm (ẩn công văn)
      this.showToTrinh = true;
      this.showCongVan = false;
      this.showDinhKem = true;
    } else {
  ...
  public isAttachmentVisible(listName: 'toTrinh' | 'congVan' | 'dinhKem'): boolean {
    switch (listName) {
      case 'toTrinh':
  ...
  // Gom tất cả file trả lời theo từng dòng để emit lên cha
  private buildReplyFilesPayload(): { tieuChiBaoCaoId: number; deMucId: number | null; fileDinhKems: CreateUpdateFileDinhKemDto[] }[] {
    const result: { tieuChiBaoCaoId: number; deMucId: number | null; fileDinhKems: CreateUpdateFileDinhKemDto[] }[] = [];
    try {
      const roots = this.dataSig();
      for (const group of roots) {
        const items = (group as any)?.children || [];
        for (const item of items) {
          const leafId = Number((item as any)?.id);
          if (Number.isNaN(leafId)) continue;
          const replies = (item as any)?.children || [];
          for (const r of replies) {
            const deMucId = (r as any)?.deMucId ?? null;
            const files = Array.isArray((r as any)?.filesReply) ? (r as any).filesReply : [];
            if (files.length) {
              const fileDinhKems: CreateUpdateFileDinhKemDto[] = files.map((f: any) => ({
                id: f?.id ? Number(f.id) : 0,
                duongDanFile: f?.url ?? f?.fileUrl ?? f?.duongDanFile ?? '',
                tenFile: f?.name ?? f?.tenFile ?? f?.fileName ?? '',
              }));
              result.push({ tieuChiBaoCaoId: leafId, deMucId, fileDinhKems });
            }
          }
        }
      }
